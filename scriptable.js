...
function createDashboardDispatcher(auth, model, web, guards) {
  ...
  const detectExperimental=async()=>{
    ...
    for(let i=0;i<kinds.length;i++){
      const kind=kinds[i],item=settled[i];
      const value=item.status==="fulfilled"?{...item.value,state:item.value&&item.value.supported===true?"available":"unavailable"}:{state:"error",supported:false,detail:cleanError(item.reason)};
      results[kind]=value; model[kind]=value; writeCapabilityCache(kind,value);
      await applyWebView(web,"zmiApplyCapability",{kind,value});
    }
    return {results,failed:kinds.filter(kind=>results[kind].state==="error")};
  };
...
function buildHtml(model) {
  ...
  const fw=model.firmwareWarning;
  const statusWarning = warnings.length ? `<div class="warning status-compatibility" data-status-warning${fw?` data-warning-id="${escapeHtml(fw.id)}"`:""}><strong>${model.errors.statusRequest ? "Status request error" : "Status compatibility warning"}</strong><p>${warnings.map(escapeHtml).join("<br>")}</p>${fw?`<button type="button" data-warning-dismiss>Acknowledge</button><details><summary>Firmware mismatch details</summary><p>Configured: ${escapeHtml(fw.configured)} · Detected: ${escapeHtml(fw.detected)}</p></details>`:""}</div>` : "";
  ...
  ${noticeHtml}
    ...
    <article class="card experimental" id="routerExperimental"><small>Experimental router controls</small><h2>USSD and device access</h2><div class="inline-toolbar"><button type="button" data-detect-experimental>Detect experimental features</button><span data-detection-status>Status: background detection pending</span></div><section data-ussd-section><h3>USSD: ${stateLabel(model.ussd)}</h3><p>${escapeHtml(model.errors.ussd || model.ussd.detail || "")}</p><button type="button" onclick="toggleUssdComposer(true)"${capabilityState(model.ussd)==='available'?'':' disabled title="Status unavailable; retry detection"'}>${capabilityState(model.ussd)==='available'?'Dial USSD':'Dial USSD — Status unavailable'}</button><form id="ussdComposer" class="composer" onsubmit="submitUssdInline(event)" hidden><input name="code" placeholder="Code, for example *100#"><div><button class="primary" type="submit">Send USSD</button><button type="button" onclick="toggleUssdComposer(false)">Cancel</button></div><p class="formStatus" data-status></p></form></section><section data-device-access-section><h3>Device access: ${stateLabel(model.deviceAccess)}</h3><p>${escapeHtml(model.errors.deviceAccess || model.deviceAccess.detail || "")}</p><div class="inline-toolbar">${deviceActions}</div>${deviceConfirm}</section><section data-cellular-control-section><h3>Cellular control: ${stateLabel(cellular)}</h3><p>${escapeHtml(cellular.detail||'Detection determines whether reconnect and mode selection are safe.')}</p></section></article>
    <article class="card"><small>Power</small><h2>System commands</h2><button class="buttonlike" type="button" data-power-action="reboot">Restart</button> <button class="danger buttonlike" type="button" data-power-action="powerOff">Power off</button>${powerConfirmCard}</article></section></main>
