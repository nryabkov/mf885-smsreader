...
function buildHtml(model) {
  ...
  ${noticeHtml}
    ...
    <article class="card experimental experimental-features" id="routerExperimental"><small>Unconfirmed firmware features</small><h2>Experimental features</h2><p>Availability depends on the router firmware. Check all features together before using them.</p><div class="inline-toolbar"><button type="button" data-detect-experimental>Detect experimental features</button><span role="status" aria-live="polite" data-detection-status>Detection has not started</span></div>
      <section class="experimental-subsection" data-cellular-control-section><h3>Cellular controls: ${stateLabel(cellular)}</h3><p>${escapeHtml(cellular.detail||'Detection determines whether reconnect and mode selection are safe.')}</p>${cellularReconnect}<h4>Preferred protocol control</h4>${cellularModeSelect}<div data-cellular-confirm hidden></div></section>
      <section class="experimental-subsection" data-ussd-section><h3>USSD: ${stateLabel(model.ussd)}</h3><p>${escapeHtml(model.errors.ussd || model.ussd.detail || "")}</p><button type="button" onclick="toggleUssdComposer(true)"${capabilityState(model.ussd)==='available'?'':' disabled title="Status unavailable; retry detection"'}>${capabilityState(model.ussd)==='available'?'Dial USSD':'Dial USSD — Status unavailable'}</button><form id="ussdComposer" class="composer" onsubmit="submitUssdInline(event)" hidden><input name="code" placeholder="Code, for example *100#"><button class="primary" type="submit">Send USSD</button></form></section>
      <section class="experimental-subsection" data-device-access-section><h3>Device access: ${stateLabel(model.deviceAccess)}</h3><p>${escapeHtml(model.errors.deviceAccess || model.deviceAccess.detail || "")}</p><div class="inline-toolbar">${deviceActions}</div>${deviceConfirm}</section>
    </article>
