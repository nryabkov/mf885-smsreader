...
function normalizeUptime(value) {
  if (typeof value === "string" && /<[^>]+>/.test(value)) value=firstText(value,["uptime","up_time","system_uptime","sys_uptime","run_time","running_time","work_time"]);
  ...
  const clock=raw.match(/^(?:(\d+)\s*(?:d|days?)\s*)?(\d{1,3}):(\d{1,2}):(\d{1,2})$/i);
  if (clock) return Number(clock[1]||0)*86400+Number(clock[2])*3600+Number(clock[3])*60+Number(clock[4]);
  const units=raw.match(/^(?:(\d+)\s*(?:d|days?)\s*)?(?:(\d+)\s*(?:h|hours?)\s*)?(?:(\d+)\s*(?:m|minutes?)\s*)?(?:(\d+)\s*(?:s|seconds?)\s*)?$/i);
  if (units && units.slice(1).some(part=>part!==undefined)) return Number(units[1]||0)*86400+Number(units[2]||0)*3600+Number(units[3]||0)*60+Number(units[4]||0);
  return null;
}
function formatUptime(value) {
  const seconds=normalizeUptime(value);
...
async function loadModel(auth) {
  ...
  try {
    ...
    model.uptime=normalizeUptime(status);
...
function buildHtml(model) {
  ...
  const uptimeLabel = formatUptime(model.uptime);
  ...
  <section class="power-toolbar" data-power-control><span>Uptime: <strong>${escapeHtml(uptimeLabel)}</strong></span><button class="danger" type="button" data-power-action="powerOff">Power off</button><div class="warning" data-power-confirm hidden></div></section>
