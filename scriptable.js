...
function redactDebugValue(value) {
  ...
  text = text.replace(/<(content|message_content|subject|contacts|from|phone_number|sender|recipient|password|passwd|pwd|pin|puk|psk|ussd(?:_code|code)?)\b[^>]*>[\s\S]*?<\/\1>/gi, "<$1><redacted></$1>");
...
function redactDebugPayload(payload) {
  ...
  const blocked = /authorization|cookie|password|passwd|pwd|token|nonce|cnonce|response|content|contacts|phone|sender|recipient|ussd/i;
...
async function loadModel(auth) {
  ...
  try {
    ...
    if(versionWarning) {
      ...
      model.firmwareWarning={id:firmwareWarningId(actualFirmware,CONFIGURED_PROFILE),configured:CONFIGURED_PROFILE,detected:actualFirmware,text:versionWarning};
...
function parseNetwork(xml, profile = ACTIVE_PROFILE) {
  ...
  const currentNames=["network_type","network_mode","ConnType","proto","sys_mode","sys_submode"];
  const names=["connect_disconnect","NW_register_status",...currentNames,"connect_mode","preferred_mode","LWG_flag","roaming"];
  const raw={}; for(const name of names) raw[name]=firstText(source,[name])||null;
  const mappings=profile&&profile.wan&&profile.wan.mappings||{}, fields={}; for(const name of names) fields[name]=enumRaw(raw[name],mappings[name]);
  const candidates=currentNames.filter(name=>raw[name]!==null).map(name=>({field:name,raw:raw[name],...networkProtocol(raw[name],profile,name)}));
  const confirmed=candidates.filter(x=>x.confirmed), generations=[...new Set(confirmed.map(x=>x.generation).filter(x=>x!=="Unknown"))];
  const conflict=generations.length>1; const chosen=conflict?null:confirmed[0]||candidates[0]||null;
  const mode=conflict?"Conflicting network data":chosen?chosen.protocol:"Unknown";
  const rawRssi=firstText(cellular,["rssi"]), operator=firstText(source,["network_name","ISP_name","home_operator","roaming_operator"]);
  const preferredHit=["preferred_mode","connect_mode"].find(name=>raw[name]!==null); const preferred=preferredHit?networkProtocol(raw[preferredHit],profile,preferredHit):null;
  const signal=signalInfo(rawRssi,null,null);
  return {hasData:Object.values(raw).some(v=>v!==null)||!!operator||rawRssi!=="",raw,fields,operator,mode,protocol:mode,generation:conflict?"Unknown":chosen&&chosen.generation||"Unknown",networkConflict:conflict,
    rawMode:chosen&&chosen.raw||candidates.map(x=>x.raw).join(" / ")||null,networkSource:conflict?candidates.map(x=>x.field).join(" / "):chosen&&chosen.field||null,networkDiagnostic:candidates.length?(conflict?"conflict: ":"")+candidates.map(x=>`${x.field}=${x.raw}`).join(", "):"No RAT field returned",preferredMode:preferred?preferred.protocol:(preferredHit?`Unknown (raw: ${raw[preferredHit]})`:"Unknown"),preferredSource:preferredHit||null,
    ...
    lac:firstText(source,["lac","LAC","tac","TAC"])||null,cellId:firstText(source,["cell_id","Cell_ID","cellid"])||null,pci:firstText(source,["pci","PCI"])||null,
    ...
    pinAttempts:firstText(source,["PIN_attempts","pin_attempts"])||null,pukAttempts:firstText(source,["PUK_attempts","puk_attempts"])||null,pdp:{context:firstText(source,["pdp_context","PDP_context"])||null,automatic:firstText(source,["automatic_pdp_list","auto_pdp_list"])||null}};
}
...
function buildHtml(model) {
  ...
  const topCards = `<section class="topgrid router-only">
    <article class="mini"><span>Signal</span><strong data-network-signal>${signalHtml}</strong><small><span data-network-current>${escapeHtml(network.networkError || network.mode || "Unknown")}</span><span data-network-dbm>${network.dbm === null || network.dbm === undefined ? "" : ` · ${escapeHtml(network.dbm)} dBm`}</span></small></article>
    <article class="mini"><span>Battery</span><strong data-battery-percent>${batteryLabel}</strong><small data-battery-inline>${escapeHtml(batteryInline)}</small></article>
    <article class="mini"><span>Traffic</span><strong data-traffic-total>${totalTraffic}</strong><small>Downloaded: <span data-traffic-down>${formatBytes(traffic.download)}</span> · Uploaded: <span data-traffic-up>${formatBytes(traffic.upload)}</span></small></article>
  </section>`;
  ...
  ${noticeHtml}
    ...
    <section id="router" class="tab${routerActive}">${statusWarning}<h2 class="section-title">Overview</h2>${topCards}
...
var remaining=model.poll,paused=false,timer=null,pending={},sequence=0,detectionAttempt=0,detectionStarted=0,detectionCompleted=0,detectionTotal=3,detectionTimer=null,lastDiagnostics={values:{},stages:{},loadedAt:null};
...
function tab(name){name=name==='router'?'router':'sms';var current=document.querySelector('.tab.active');if(current)safeStorageSet('zmiScrollY:'+current.id,String(window.scrollY||0));document.querySelectorAll('.tab').forEach(function(x){x.classList.toggle('active',x.id===name)});document.querySelectorAll('[data-tab-button]').forEach(function(x){x.classList.toggle('active',x.getAttribute('data-tab-button')===name)});safeStorageSet('zmiTab',name);var saved=safeStorageGet('zmiScrollY:'+name);setTimeout(function(){window.scrollTo(0,saved===null?0:Number(saved)||0)},0)}
...
function detectExperimental(button){button=button||document.querySelector('[data-detect-experimental]');if(!button)return Promise.resolve();detectionAttempt++;detectionStarted=Date.now();detectionCompleted=0;detectionTotal=3;drawDetectionProgress();if(detectionTimer)clearInterval(detectionTimer);detectionTimer=setInterval(drawDetectionProgress,1000);button.disabled=true;button.textContent='Detecting…';button.setAttribute('aria-busy','true');return bridge('detectExperimental',{},button).then(function(r){var failed=r&&r.failed||[],elapsed=detectionElapsed(),status=document.querySelector('[data-detection-status]');detectionCompleted=r&&r.completed==null?detectionTotal:r.completed;if(status)status.textContent='Attempt '+detectionAttempt+(failed.length?' failed after ':' completed in ')+elapsed+'s · '+detectionCompleted+'/'+detectionTotal+' checks';button.textContent=failed.length?'Retry experimental detection':'Detect experimental features'}).catch(function(e){var status=document.querySelector('[data-detection-status]');if(status)status.textContent='Attempt '+detectionAttempt+' failed after '+detectionElapsed()+'s · '+detectionCompleted+'/'+detectionTotal+' checks';button.textContent='Retry experimental detection';showActionError('Detection failed',describeError(e),'')}).finally(function(){if(detectionTimer){clearInterval(detectionTimer);detectionTimer=null}button.disabled=false;button.removeAttribute('aria-busy')})}
