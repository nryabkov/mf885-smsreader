...
module.exports = { run, parseDigestChallenge, buildHtml, clientScript };
...
function buildHtml(model) {
  ...
  <nav class="seg"><button data-tab-button="sms" class="${smsActive.trim()}" onclick="tab('sms')">SMS</button><button data-tab-button="router" class="${routerActive.trim()}" onclick="tab('router')">Роутер</button></nav>
  ...
  <section id="webviewDiagnostics" class="action-status warning" hidden><header><strong>Ошибка интерфейса WebView</strong><button type="button" onclick="hideWebviewDiagnostics()">Закрыть</button></header><p>Ошибка интерфейса WebView. Откройте детали ниже или обновите скрипт.</p><pre data-webview-diagnostics></pre></section>
  ${model.notice ? `<div class="notice">${escapeHtml(model.notice)}</div>` : ""}
    <section id="sms" class="tab${smsActive}"><div class="inline-toolbar"><button onclick="toggleSmsComposer()">📝 Написать SMS</button><button onclick="toggleUssdComposer()">☎️ Набрать USSD</button></div>
...
window.onunhandledrejection=function(event){var reason=event&&event.reason?event.reason:event;showActionError('Ошибка JavaScript в WebView',describeError(reason),'');showWebviewDiagnostics('Unhandled promise rejection','',0,0,reason);};
