...
async function run(options = {}) {
  ...
  // The shipped legacy compatibilityProfile value is not evidence that the
  // user intentionally pinned a firmware. New pins use the explicit override.
  if (!PROFILE_OVERRIDE && CONFIGURED_PROFILE !== LEGACY_DEFAULT_PROFILE) PROFILE_OVERRIDE = CONFIGURED_PROFILE;
  ACTIVE_PROFILE = compatibilityModule.selectProfile(PROFILE_OVERRIDE || "");
...
async function loadModel(auth) {
  ...
  try {
    ...
    if (!PROFILE_OVERRIDE && actualFirmware) ACTIVE_PROFILE=compatibilityModule.selectProfile(actualFirmware);
...
function parseNetwork(xml, profile = ACTIVE_PROFILE) {
  ...
  const connected=/^(?:1|connected|up|active)$/i.test(String(raw.connect_disconnect||""));
  const combinationRule=(profile&&profile.wan&&profile.wan.currentRatRules||[]).find(rule=>Object.keys(rule.fields||{}).every(name=>String(raw[name])===String(rule.fields[name]))&&(rule.connected===undefined||rule.connected===connected));
  ...
  if(combinationRule)candidates.unshift({field:combinationRule.source||"firmware combination rule",raw:Object.keys(combinationRule.fields).map(name=>`${name}=${raw[name]}`).join(","),protocol:combinationRule.label,generation:ratGeneration(combinationRule.label),confirmed:true,combination:true});
  const confirmed=candidates.filter(x=>x.confirmed), generations=[...new Set(confirmed.map(x=>x.generation).filter(x=>x!=="Unknown"))];
  // A confirmed combination rule describes the whole status snapshot and wins
  // over isolated codes. Otherwise sys_mode precedes sys_submode by field order.
  const conflict=!combinationRule&&generations.length>1; const chosen=conflict?null:confirmed[0]||candidates[0]||null;
  ...
  return {hasData:Object.values(raw).some(v=>v!==null)||!!operator||rawRssi!=="",raw,fields,operator,mode,protocol:mode,generation:conflict?"Unknown":chosen&&chosen.generation||"Unknown",networkConflict:conflict,
    rawMode:combinationRule?Object.keys(combinationRule.fields).map(name=>`${name}=${raw[name]}`).join(", "):chosen&&chosen.raw||candidates.map(x=>x.raw).join(" / ")||null,networkSource:conflict?candidates.map(x=>x.field).join(" / "):chosen&&chosen.field||null,networkDiagnostic:candidates.length?(conflict?"conflict: ":"")+candidates.filter(x=>!x.combination).map(x=>`${x.field}=${x.raw}`).join(", "):"No RAT field returned",preferredMode:preferred?preferred.protocol:(preferredHit?`Unknown (raw: ${raw[preferredHit]})`:"Unknown"),preferredSource:preferredHit||null,
