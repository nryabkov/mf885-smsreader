...
- [ZIMI / RestoreFw reverse engineering](RESTOREFW_REVERSE.md) — device-bound AES header, exact partition/checksum/archive formats, native multipart and restore state machine, the quarantined Canary r3 defect, and the bounded-risk no-dump route.
- [SafeFlash Stage 0](FIRMWARE_FLASH_SAFETY.md) — destructive gates, one-shot journal semantics, and current production lock state.
- [Firmware ladder](../FIRMWARE_LADDER.md) — the one-variable sequence from observer-only WEBI logging through SMS, USSD, TTL and finally IMEI, with a golden rollback at every gate.
...
For command-like models, **semantics**, **request transport**, **project implementation**, and **live side effect** are separate claims. MF885 2.5.94 reboot now has firmware-confirmed semantics, companion-app-confirmed GET transport, and implemented project support; live execution on the exact device remains a distinct evidence level.
...
The dedicated MF885 2.5.94 runtime profile now implements the APK-confirmed forms:

```text
Restart   -> GET /xml_action.cgi?method=get&module=duster&file=reset
Power off -> GET /xml_action.cgi?method=get&module=duster&file=poweroff
```

Both carry no XML request body. The retained 2.5.96 mapping remains independent.
...
- **MF885 2.5.94** — exact stock BackupFw/raw OSLO fingerprints and native Duster work are recorded in [MF885_2.5.94_STATIC_ANALYSIS.md](MF885_2.5.94_STATIC_ANALYSIS.md); a period-relevant ZMI Android 1.2.42 client confirms GET/no-body power transport for MF885/MF96 Ver.D-family devices, and that transport is now implemented by the project;
