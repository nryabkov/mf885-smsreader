...
The project does not currently expose factory reset as a dashboard control.
...
5. Encode destructive transport per profile; do not assume every power command uses POST.
6. Never blindly retry reboot/shutdown after connection loss.
...
The API helper is transport-aware for destructive commands. A model descriptor can carry `method: GET`, in which case the command is submitted through the existing GET path and the semantic XML tree is not used as a request body. String descriptors preserve the existing POST/SET behaviour.

For the dedicated MF885 2.5.94 profile this means:

```text
Restart   -> GET method=get file=reset, no body
Power off -> GET method=get file=poweroff, no body
```
