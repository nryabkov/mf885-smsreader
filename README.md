...
The report contains only the selected file name, size/hash, device/power gates, and explicit zero-write counters; it does not retain the selected bytes or expose the full Files path. Passing validation means only that the image and live preconditions are ready for transport-capture work. The `RestoreFw` transport and physical-recovery allowlists remain empty, `firmwarePostsAttempted` is always `0`, and `flashAllowed` is always `false`.

### Stage 0 firmware restore

The dashboard now contains a distinct **Firmware restore (Stage 0)** control, backend transaction flow, and **Stage 0 journal** viewer. It is disabled with its current blockers until both a complete, reviewed RestoreFw contract and physical recovery record are compiled. The contract must bind every wire-significant upload/authentication/acceptance/status field, bounded GET-only polling, the reviewed adapter artifact hash, and a platform-backed cross-process exclusive-lease profile. The recovery record must bind three identical full 32 MiB dumps of the 1.8 V NOR, the exact golden backup, a verified recovery-mode entry, and a privacy-safe fingerprint of the one physical router being flashed. No production transport adapter is present in this build; test injection cannot be reached from WebView.

Once those records and the core-owned adapter exist, Scriptable keeps immutable native `Data`, finishes both hashes before arming, obtains the final live identity/power from the prepared session, and requires a typed native `FLASH <sha-prefix>` confirmation. `POST_ARMED` is saved and read back from Keychain, and the exclusive lease is rechecked before the sole Request. There are no redirects or automatic retries. A timeout, disconnect, unknown status, or restart after arming leaves a terminal `UNKNOWN` journal. Status and boot polling are GET-only and bounded. Firmware mode pauses WebView polling and rejects other router actions. The first qualification is golden-to-golden; Canary stays blocked until that exact transaction reaches `BOOT_VERIFIED` under the same transport, recovery, and unit fingerprint. Completed `BOOT_VERIFIED`/explicit `FAILED` journals require acknowledgement before another run; `UNKNOWN` is never normally clearable.
