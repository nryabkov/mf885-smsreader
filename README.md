...
The dashboard now contains a distinct **Firmware restore (Stage 0)** control, backend transaction flow, and **Stage 0 journal** viewer. It is disabled with its current blockers until both a complete, reviewed RestoreFw contract and a compiled risk/recovery record exist. The current code supports only the preferred physical record (three identical full 32 MiB dumps of the 1.8 V NOR plus verified recovery entry), so its allowlist remains empty. The documented no-dump route requires a separate `software-only-risk-v1` schema and explicit no-recovery acceptance; it is not a boolean override and is not implemented yet. The transport must bind every wire-significant upload/authentication/acceptance/status field, bounded GET-only polling, the reviewed adapter artifact hash, and a platform-backed cross-process exclusive-lease profile. No production transport adapter is present in this build; test injection cannot be reached from WebView.
