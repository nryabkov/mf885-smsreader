...
The separate VDS qualification harness in `tools/mf885_vds_restore*.js` does not unlock that Scriptable control. It is a single-purpose sender hard-bound to this one reviewed MF885 fingerprint, `192.168.21.1:80`, the exact stock-golden payload and deterministic multipart-body hashes, and `Action=RestoreFw`. Private captures, settings, credentials and the durable transaction journal stay under ignored `.runtime/` files with modes `0700`/`0600`. Before the router request it burns a permanent local allowance and creates one deterministic GitHub tag with the create-reference API. Only an exact HTTP 201 proving creation of that previously absent tag permits the core-owned HTTP sender to make one request; an existing tag, timeout, lost response, stale Digest session, state corruption or any non-exact reply permanently blocks replay. This is an **at-most-one POST-attempt** guarantee under the trusted VDS runtime and the requirement that the fence tag is never deleted or rewritten—not an exactly-once delivery or flash guarantee.

`node tools/mf885_vds_restore.js readiness` checks the two byte-identical backups, recomputed private Wi-Fi/APN evidence, exact multipart and absent remote fence without contacting the router. `preflight` performs the final bounded live identity/power/idle-status/session checks with zero firmware POSTs. The original destructive command additionally required the exact `--confirm=FLASH-GOLDEN-2b5880fc` argument. `status` and `monitor` are recovery tools; after an accepted upload, status and reboot-boundary observation use bounded GETs, while the final health proof adds one semantic SMS-history read POST with zero message mutation. Neither path replays `RestoreFw`.

The first allowed live qualification attempt was consumed on 2026-08-21. The permanent local allowance and remote fence were committed, exactly one golden `RestoreFw` POST was dispatched, and the HTTP reply failed the exact native acceptance predicate. The transaction is terminal `UNKNOWN`, `postAllowed=false`; neither the tool nor an operator acknowledgement can replay it. For the following two minutes the exact router remained continuously reachable on the same stock firmware and returned restore status `0/0/No Error!`, with no reboot boundary, so no successful golden qualification was recorded. Canary upload therefore remains locked.
